...
export default function AdminBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (response.success && response.data) {
        const data = response.data as { bookings?: any[], total?: number }
        console.log('Bookings data:', data)
        setBookings(data.bookings || [])
        setTotal(data.total || 0)
        setError(null)
      } else {
        const errorMsg = response.error || 'Failed to fetch bookings'
  ...
  const handleStatusChange = async (id: string, newStatus: 'pending' | 'confirmed' | 'cancelled' | 'completed') => {
    try {
      const response = await apiClient.updateBooking(id, { status: newStatus })
      if (response.success) {
        fetchBookings()
        refreshDashboard()
      } else {
  ...
  const handleFilterChange = (newFilter: 'all' | 'pending' | 'confirmed' | 'cancelled' | 'completed') => {
    setFilter(newFilter)
    setCurrentPage(1)
  }
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="text-[#d1a058]">Loading bookings...</div>
      ) : error ? (
        <div className="bg-red-500/20 border-2 border-red-500/50 rounded-lg p-6">
          <div className="text-red-400 font-semibold mb-2">Error loading bookings</div>
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="text-[#d1a058] text-sm mb-1">{booking.email}</div>
                      <div className="text-white/80 text-sm mb-1">Mobile: {booking.mobile}</div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className={`px-2 py-1 rounded text-xs border transition-all ${getStatusColor(booking.status)}`}
                      >
                        <option value="pending">Pending</option>
                        <option value="confirmed">Confirmed</option>
                        <option value="cancelled">Cancelled</option>
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-white/60 text-sm">
                Showing {(currentPage - 1) * ITEMS_PER_PAGE + 1} to {Math.min(currentPage * ITEMS_PER_PAGE, total)} of {total} bookings
              </div>
