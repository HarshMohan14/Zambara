...
import { collection, getDocs, updateDoc, doc, query, where, orderBy, limit, Timestamp } from 'firebase/firestore'
...
export async function POST(req: Request) {
  try {
    ...
    // Fetch all pending users and sort in memory to avoid composite index requirement
    const pendingQuery = query(tagconRef, where('status', '==', 'pending'))
    ...
    // 2. Count existing tribes to ensure balanced distribution
    const snapshot = await getDocs(tagconRef)
    ...
    // 3. Update the user document to completed with their assigned tribe
