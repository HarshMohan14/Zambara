...
export default function ZambaaraTournamentPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error loading Zambaara tournaments:", error)
    })
    return () => unsubscribe()
  }, [])
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error loading Zambaara bookings:", error)
    })
    return () => unsubscribe()
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo('.zambaara-content-anim', 
        { opacity: 0, y: 30 },
  ...
  return (
    <>
      ...
      >
        ...
        {/* Dynamic golden aura */}
        <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_center,rgba(209,160,88,0.06)_0%,transparent_70%)] z-0" />
        ...
        <div className="relative z-20 container mx-auto px-4 max-w-7xl flex flex-col">
          ...
          {/* Header section */}
          <div className="text-center mb-10 zambaara-content-anim">
            <h1 
          ...
          {/* Selector Bar */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[#d1a058]/20 mb-8 max-w-5xl mx-auto w-full zambaara-content-anim">
            <div className="flex-1 max-w-sm">
              <label className="block text-xs font-semibold uppercase text-white/50 mb-1.5">Select active tournament</label>
              {tournaments.length === 0 ? (
                <p className="text-[#ff4400] text-sm font-semibold uppercase">No tournaments currently registered.</p>
              ) : (
                <select
          ...
          {!activeTourney ? (
            <div className="text-center py-20 text-white/40 uppercase font-black tracking-widest zambaara-content-anim">Select a tournament to view details</div>
          ) : (
            <div className="flex-1 flex flex-col gap-10">
              ...
              <div className="w-full max-w-5xl mx-auto zambaara-content-anim">
                      ...
                      <div key={tr.id} className="bg-black/55 border border-white/5 rounded-2xl p-6 shadow-lg">
                        ...
                        <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
                          ...
                          <span className="text-[10px] text-white/50 uppercase font-semibold">
                            Booked: {tribeRoster.length} / {capacity}
                          </span>
                        </div>
                            ...
                            {tribeRoster.map((player) => (
                              ...
                              >
                                <div>
                                  ...
                                  <p className="text-[10px] text-white/45 font-mono mt-0.5">Seat #{ (player.seatIndex % capacity) + 1 }</p>
                                </div>
