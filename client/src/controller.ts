...
import { GAME_CONFIG, Vector3Data } from 'shared';
...
export class PlayerController {
  ...
  private targetBeta: number = Math.PI / 3;
  ...
  private setupInputs(): void {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.code === 'ControlLeft' || e.code === 'KeyC') this.isCrouching = true;
    });

    window.addEventListener('keyup', (e) => {
      this.keys[e.code] = false;
      if (e.code === 'ShiftLeft' || e.code === 'ShiftRight') this.isSprinting = false;
      if (e.code === 'ControlLeft' || e.code === 'KeyC') this.isCrouching = false;
    });
   ...
   */
  public update(playerMesh: Mesh, dt: number): {
    position: Vector3Data;
    rotationY: number;
    speedMode: 'walk' | 'sprint' | 'crouch';
  } {
    ...
    let speedMode: 'walk' | 'sprint' | 'crouch' = 'walk';
    if (this.isSprinting && !this.isCrouching) speedMode = 'sprint';
    if (this.isCrouching) speedMode = 'crouch';
    ...
    if (this.keys['KeyW'] || this.keys['ArrowUp']) moveDirection.addInPlace(forward);
    if (this.keys['KeyS'] || this.keys['ArrowDown']) moveDirection.addInPlace(forward.scale(-1));
    if (this.keys['KeyD'] || this.keys['ArrowRight']) moveDirection.addInPlace(right.scale(-1));
    if (this.keys['KeyA'] || this.keys['ArrowLeft']) moveDirection.addInPlace(right);
