import { Engine, Scene, Vector3, Mesh, MeshBuilder, StandardMaterial, Color3, Scalar, Matrix, ArcRotateCamera, HemisphericLight, DirectionalLight, AbstractMesh } from '@babylonjs/core';
const BABYLON = {
    Engine,
    Scene,
    Vector3,
...
import { PlayerRole, PlayerStatus, DoorState, GAME_CONFIG, MatchPhase } from 'shared';
...
export class GameEngine {
    ...
    // Local player
    localMesh = null;
    localPlayerRole = PlayerRole.Civilian;
    ...
    updateLocalPlayer(dt) {
        ...
        if (this.isHidden) {
            ...
            if (now - this.lastMoveSendTime >= 33) {
                ...
                network.sendMove({ x: this.localMesh.position.x, y: this.localMesh.position.y, z: this.localMesh.position.z }, this.localMesh.rotation.y, 'walk', elapsedSeconds);
            }
            return;
        }
        // Run WASD movement
        ...
        if (now - this.lastMoveSendTime >= 33) {
            const elapsedSeconds = this.lastMoveSendTime === 0 ? dt : (now - this.lastMoveSendTime) / 1000;
            this.lastMoveSendTime = now;
            network.sendMove(moveReport.position, moveReport.rotationY, moveReport.speedMode, elapsedSeconds);
        }
    }
    ...
    updateInteractions() {
        ...
        GAME_CONFIG.map.doors.forEach(door => {
            ...
            if (dist <= range) {
                ...
                const state = network.socket?.io ? network.lastState?.doors[door.id] || DoorState.Closed : DoorState.Closed;
                let label = '';
                if (state === DoorState.Open)
                    ...
                    if (this.localPlayerRole === PlayerRole.Attacker)
                        label = "Hold: Break Door Lock";
                    else if (this.localPlayerRole === PlayerRole.RescueOfficer)
                        label = "Hold: Breach Door Lock";
                    else
                        label = "Locked Door";
                }
        ...
        this.remotePlayers.forEach(p => {
            ...
            if (dist <= range) {
                ...
                if (this.localPlayerRole === PlayerRole.Attacker && p.role === PlayerRole.Civilian && p.status === PlayerStatus.Active) {
                    label = `Hold: Capture ${network.socket?.io ? network.lastState?.players.find((pl) => pl.id === p.id)?.displayName || 'Civilian' : 'Civilian'}`;
                }
    ...
    handleEPressed() {
        ...
        if (target.type === 'door') {
            const state = network.lastState?.doors[target.id] || DoorState.Closed;
    ...
    startHoldInteraction(type, targetId, duration) {
    ...
    triggerPulseAttack() {
        if (this.localPlayerRole !== PlayerRole.RescueOfficer || this.localPlayerStatus !== PlayerStatus.Active)
            return;
        if (!this.controller)
            return;
        const camera = this.controller.getCamera();
    ...
    triggerNoiseIndicator(noisePos) {
        if (!this.localMesh)
            return;
    ...
    updateFloatingTags() {
        ...
        state.players.forEach(p => {
            ...
            if (p.id === network.playerId && !this.isSpectating)
                return;
            ...
            const screenPos = BABYLON.Vector3.Project(tagPos3D, BABYLON.Matrix.Identity(), this.scene.getTransformMatrix(), viewport);
            ...
            if (isBehind) {
                ...
                if (p.role === PlayerRole.Attacker)
                    colorVar = 'var(--attacker)';
                if (p.role === PlayerRole.RescueOfficer)
                    colorVar = 'var(--rescue)';
                if (p.status === PlayerStatus.Captured)
                    colorVar = 'var(--captured)';
