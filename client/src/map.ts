...
export class GameMap {
  ...
  public doorMeshes: Map<string, Mesh> = new Map();
  public hidingSpotMeshes: Map<string, Mesh> = new Map();
  private extractionDisc: Mesh | null = null;
  private extractionGate: Mesh | null = null;
  ...
  public updateDoors(doorStates: Record<string, DoorState>): void {
    GAME_CONFIG.map.doors.forEach(doorConfig => {
  ...
  public getCollidables(): AbstractMesh[] {
    const collidables: AbstractMesh[] = [...this.wallMeshes];
    
    this.doorMeshes.forEach((pivot, id) => {
      if (pivot.isEnabled()) {
        const slab = pivot.getChildren()[0] as Mesh;
        if (slab) collidables.push(slab);
      }
    });
