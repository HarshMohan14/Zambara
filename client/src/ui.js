...
import { PlayerRole, PlayerStatus, MatchPhase, calculateExtractionTarget } from 'shared';
export class UIManager {
    ...
    `;
        ...
        const selectRole = document.getElementById('select-role-pref');
    ...
    renderGameHUD(state) {
      ...
      `;
        ...
        let phaseName = 'Match';
        if (state.phase === MatchPhase.CivilianPreparation)
            phaseName = 'Civilian Prep';
        if (state.phase === MatchPhase.AttackerEntry)
            phaseName = 'Attacker Hunt';
        if (state.phase === MatchPhase.RescuePhase)
            phaseName = 'Rescue Phase';
        if (state.phase === MatchPhase.MatchEnd)
            phaseName = 'Match Ended';
        // Combat indicators
    ...
    renderDebugPanelHtml(state, localPlayer) {
        const isAdmin = state.adminId === network.playerId;
    ...
    setupDebugPanelListeners(state) {
        const isAdmin = state.adminId === network.playerId;
        if (isAdmin) {
    ...
    showInteractionPrompt(key, label) {
    ...
    showInteractionProgress(key, label, currentMs, totalMs) {
        const zone = document.getElementById('hud-interaction-zone');
        if (!zone)
            return;
    ...
    renderMatchEndScreen(winResult, stats) {
        this.currentScreen = 'lobby';
        let statsGridHtml = '';
        let playerRowsHtml = '';
        if (stats) {
      ...
      `;
            playerRowsHtml = Object.entries(stats.playerStats).map(([id, pStats]) => {
                ...
                if (role === PlayerRole.Attacker)
                    roleStyle = 'color: var(--attacker)';
                if (role === PlayerRole.RescueOfficer)
                    roleStyle = 'color: var(--rescue)';
                return `
          <tr>
            <td><b>${name}</b></td>
