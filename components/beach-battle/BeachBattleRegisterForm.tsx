...
export function BeachBattleRegisterForm() {
  ...
  const revealPhase1Ref = useRef<HTMLDivElement>(null)
  const revealPhase2Ref = useRef<HTMLDivElement>(null)
  const revealPhase3Ref = useRef<HTMLDivElement>(null)
  ...
  // ── SIMPLE REVEAL ANIMATION (3 clear phases, mobile-friendly) ──
  const playRevealAnimation = useCallback(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Phase 1: Fade in container + "Summoned by the Tides" text with wave dots
      tl.fromTo(revealContainerRef.current,
        { opacity: 0 },
        { opacity: 1, duration: 0.5 }
      )

      if (revealPhase1Ref.current) {
        tl.fromTo(revealPhase1Ref.current,
          { opacity: 0, y: 20 },
          { opacity: 1, y: 0, duration: 0.6 },
          '-=0.2'
        )
        // Hold for suspense
        tl.to(revealPhase1Ref.current, { duration: 2 })
        // Fade out phase 1
        tl.to(revealPhase1Ref.current,
          { opacity: 0, y: -15, duration: 0.4 }
        )
      }

      // Phase 2: Tribe icon + name burst in
      if (revealPhase2Ref.current) {
        tl.fromTo(revealPhase2Ref.current,
          { opacity: 0, scale: 0.7 },
      ...
      // Phase 3: Card + actions slide up
      if (revealPhase3Ref.current) {
        tl.fromTo(revealPhase3Ref.current,
          { opacity: 0, y: 40 },
          { opacity: 1, y: 0, duration: 0.6 },
          '+=0.3'
        )
  ...
  useEffect(() => {
    if (formState === 'revealing') {
      const t = setTimeout(() => playRevealAnimation(), 150)
    ...
    try {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: `The tides summoned me to ${assignedTribe?.name} tribe at Zambaara Beach Battle! \uD83C\uDF0A\u2694\uFE0F`,
        })
      } else {
        downloadCard()
      }
  ...
  if (formState === 'closed') {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-16">
        <div ref={formRef} className="w-full max-w-md md:max-w-2xl text-center opacity-0">
          ...
          <p className="text-base md:text-lg text-white/50 mb-4 max-w-xs md:max-w-md mx-auto"
          ...
          {/* Tribe slots - 2x2 on mobile, 4-col on desktop */}
          ...
          <div className="rounded-xl p-4 mb-6 max-w-xs md:max-w-md mx-auto"
            style={{
              background: 'linear-gradient(145deg, rgba(239,68,68,0.05) 0%, rgba(0,0,0,0.4) 100%)',
              border: '1px solid rgba(239,68,68,0.15)',
            }}>
            <p className="text-sm md:text-base text-white/60" style={{ fontFamily: "'BlinkerRegular', sans-serif" }}>
  ...
  if ((formState === 'revealing' || formState === 'success') && assignedTribe && result) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-16">
        ...
        <div ref={revealContainerRef} className="w-full max-w-md md:max-w-3xl text-center" style={{ opacity: 0 }}>
          {/* Background glow */}
          ...
          {/* ── PHASE 1: "Summoned by the Tides" choosing text ── */}
          <div ref={revealPhase1Ref} className="relative z-10 min-h-[200px] flex flex-col items-center justify-center"
            style={{ display: formState === 'success' ? 'none' : 'flex' }}>
            {/* Pulsing wave icon */}
            <div className="text-5xl sm:text-6xl mb-5 animate-pulse">
              {'\uD83C\uDF0A'}
            </div>
            <p className="text-lg sm:text-xl md:text-2xl uppercase tracking-[0.15em] mb-3"
              style={{
                fontFamily: "'BlinkerSemiBold', sans-serif",
            ...
            <div className="flex justify-center gap-1.5 mt-2">
              {[0, 1, 2].map((i) => (
                <div key={i} className="w-2.5 h-2.5 rounded-full bg-cyan-400/60"
                  style={{ animation: `revealDotPulse 1.2s ease-in-out ${i * 0.3}s infinite` }} />
              ))}
            </div>
          </div>

          {/* ── PHASE 2: Tribe icon + text reveal ── */}
          <div ref={revealPhase2Ref} className="relative z-10"
            style={{ opacity: 0 }}>
            {/* Desktop: horizontal layout; Mobile: stacked */}
            <div className="md:flex md:items-center md:justify-center md:gap-8">
              {/* Tribe icon */}
              <div className="text-7xl sm:text-8xl md:text-9xl mb-4 md:mb-0 inline-block"
                style={{ filter: `drop-shadow(0 0 40px ${tc.glowColor})` }}>
                {tc.icon}
              </div>

              <div className="text-center md:text-left">
                {/* Creative reveal text */}
                ...
                {/* Tribe name */}
                <h2 className="text-5xl sm:text-6xl md:text-8xl font-bold uppercase mb-1"
                  style={{
          ...
          {/* ── PHASE 3: Warrior card + action buttons ── */}
          <div ref={revealPhase3Ref} className="relative z-10 mt-8"
            style={{ opacity: 0 }}>

            {/* Desktop: card + actions side-by-side; Mobile: stacked */}
            <div className="md:flex md:items-start md:justify-center md:gap-8">
                  ...
                  {[
                    ...
                    <div key={i}>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-base md:text-lg font-bold truncate max-w-[180px] md:max-w-[220px]"
              ...
              <div className="space-y-3 md:min-w-[260px]">
                ...
                <div className="pt-2">
                  <Link href="/beach-battle"
                    className="inline-block w-full text-center px-8 py-3.5 rounded-xl text-sm uppercase tracking-wider font-semibold transition-all duration-300 hover:scale-[1.03] active:scale-95"
                    style={{
                      fontFamily: "'BlinkerSemiBold', sans-serif",
                      background: 'linear-gradient(135deg, rgba(6,182,212,0.2) 0%, rgba(14,116,144,0.3) 100%)',
                      border: '1.5px solid rgba(6,182,212,0.4)', color: '#e0f2fe',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16 sm:py-20">
      {/* Desktop: wider container, side-by-side info+form; Mobile: single column as-is */}
      <div ref={formRef} className="w-full max-w-md md:max-w-4xl opacity-0">

        {/* Desktop layout wrapper */}
        <div className="md:flex md:items-start md:gap-12">

          {/* ── LEFT COLUMN (desktop only): Header + tribe info ── */}
          <div className="md:flex-1 md:sticky md:top-24">
            ...
            <div className="text-center md:text-left mb-6">
              <Link href="/beach-battle" className="inline-block">
                <Image src="/Zambaara.png" alt="Zambaara" width={120} height={40}
                  className="mx-auto md:mx-0 opacity-70 hover:opacity-100 transition-opacity" />
              </Link>
            </div>
            ...
            <div className="text-center md:text-left mb-8">
              ...
              <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold uppercase mb-3"
                style={{
              ...
              <p className="text-base sm:text-lg md:text-xl text-white/40 max-w-xs md:max-w-sm mx-auto md:mx-0"
                style={{ fontFamily: "'BlinkerRegular', sans-serif" }}>
              ...
              {TRIBES.map((t) => {
                ...
                return (
                  <div key={t.name} className="text-center">
                    ...
                    {/* Desktop: show tribe name below count */}
                    <p className="hidden md:block text-xs uppercase tracking-wider mt-0.5"
                      style={{ fontFamily: "'BlinkerRegular', sans-serif", color: full ? 'rgba(255,255,255,0.15)' : `${displayColor(t)}50` }}>
                      {t.name}
                    </p>
                  </div>
                )
            ...
            {/* Desktop only: decorative tagline */}
            <div className="hidden md:block mb-6">
              <div className="rounded-xl p-4"
                style={{
                  background: 'linear-gradient(145deg, rgba(6,30,50,0.3) 0%, rgba(0,0,0,0.4) 100%)',
