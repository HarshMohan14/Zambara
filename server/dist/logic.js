 ...
 */
export function assignRoles(playersInput) {
 ...
 */
export function evaluateWinConditions(players, phase, phaseTimer, attackerHealth) {
    ...
    const activeOrHiddenCount = civilians.filter(p => p.status === PlayerStatus.Active || p.status === PlayerStatus.Hidden).length;
