...
import { validateMove, isInsideStaircase } from './physics.js';
  ...
  it('should run full match lifecycle: Room -> Start -> Phase changes -> Win', () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dist = Math.sqrt(
        Math.pow(civ!.position.x - zoneCenter.x, 2) +
      ...
      if (dist <= GAME_CONFIG.extraction.zoneRadius) {
        ...
        const current = (room as any).extractionProgress.get(civ!.id) || 0;
        const next = current + dt * 1000;
        (room as any).extractionProgress.set(civ!.id, next);
        if (next >= GAME_CONFIG.interaction.holdDurations.extract) {
          civ!.status = PlayerStatus.Extracted;
          if ((room as any).matchStats) {
            (room as any).matchStats.extractedCivilians++;
          }
        }
      }
    }
